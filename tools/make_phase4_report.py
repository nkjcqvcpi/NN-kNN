...
def steps_to_threshold(evals: pd.DataFrame) -> int | None:
    hit = evals[evals["mean_return"] >= SUCCESS_THRESHOLD]
...
def build_table(data: dict) -> str:
    lines = [
        "| method | final_eval (mean ± std) | last_eval (mean ± std) | best_model_step (mean ± std) | steps-to-475 per seed (eval grid) |",
        "|---|---|---|---|---|",
    ]
    for method in METHOD_ORDER:
        ...
        thresh = [steps_to_threshold(r["evals"]) for r in runs.values()]
...
def plot_per_method(data: dict, outdir: Path) -> Path:
    ...
    for ax, method in zip(axes.flat, METHOD_ORDER):
        ...
        ax.axhline(SUCCESS_THRESHOLD, color="#5f5e56", linewidth=1.0, linestyle=(0, (4, 3)))
    ...
    fig.suptitle("CartPole-v1 fixed-budget evaluation curves — dashed line marks 475 success threshold",
                 fontsize=11, color="#1a1a19")
    fig.tight_layout(rect=(0, 0, 1, 0.97))
    out = outdir / "phase4_learning_curves_per_method.png"
    fig.savefig(out, dpi=160, facecolor="#fcfcfb")
    plt.close(fig)
    return out


def plot_means(data: dict, outdir: Path) -> Path:
    ...
    for i in range(1, len(label_pos)):
        label_pos[i][1] = max(label_pos[i][1], label_pos[i - 1][1] + 24)
    ...
    ax.axhline(SUCCESS_THRESHOLD, color="#5f5e56", linewidth=1.0, linestyle=(0, (4, 3)))
    ax.text(1, SUCCESS_THRESHOLD + 8, "475 success threshold", fontsize=8.5, color="#5f5e56")
    style_axis(ax)
    ...
    ax.set_title("CartPole-v1 — seed-mean evaluation return at matched 150k budgets (band = min–max over 5 seeds)",
                 fontsize=11, color="#1a1a19")
    fig.tight_layout()
    out = outdir / "phase4_learning_curves_mean.png"
    fig.savefig(out, dpi=160, facecolor="#fcfcfb")
    plt.close(fig)
    return out
...
def main() -> None:
    ...
    print()
    print(f"figure: {plot_per_method(data, outdir)}")
    print(f"figure: {plot_means(data, outdir)}")
